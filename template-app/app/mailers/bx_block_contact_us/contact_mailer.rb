module BxBlockContactUs
  class ContactMailer < ApplicationMailer
    ...
    def send_mail(user)
      ...
      from: @user.email, 
      subject: "Welcome to Our Site #{@user.first_name}") if admin_emails.any?
    end
    ...
    def send_profile_mail(user)
      ...
      if @profile_user.type == "ClientUser"
        ...
        subject: "Welcome to Our Site #{@profile_user.first_name}") if lf_and_client_admin_mails.any?
      else
        clients_admins = AdminUser.all
        mail(to: clients_admins.map(&:email),
        from: @profile_user.email, 
        subject: "Welcome to Our Site #{@profile_user.first_name}") if clients_admins.any?
      end
    end 
  end
end