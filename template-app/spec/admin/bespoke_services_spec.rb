...
RSpec.describe Admin::BespokeServicesController, type: :controller do
  render_views
  before(:each) do
    @admin = AdminUser.create!(email: 'test123@example.com', password: 'password', password_confirmation: 'password')
    @admin.save
    @company = FactoryBot.create(:company)
    @service = FactoryBot.create(:service, company_id: @company.id)
    3.times do
